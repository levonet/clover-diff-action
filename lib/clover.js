            ...
            pckge.children
                ...
                .forEach((file) => {
                    const metrics = file.children
                        ...
                        .map((node) => node.children)[0]
                        .filter((node) => node.tagName === 'metrics')[0].attributes
                    result.testruns += parseInt(metrics.testruns || 0, 10)
                    result.testpasses += parseInt(metrics.testpasses || 0, 10)
                    result.testfailures += parseInt(metrics.testfailures || 0, 10)
                    result.testduration = Math.round(result.testduration + parseFloat(metrics.testduration || 0) * 1000)
                })
        })
